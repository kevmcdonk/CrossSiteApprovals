...
namespace SharePoint.WebHooks.Common
{
    ...
    public static class ContextProvider
    {
        ...
        /// <param name="siteUrl">Url to create the client context for</param>
        /// <returns>Created client context</returns>
        public async static Task<ClientContext> GetWebApplicationClientContext(String siteUrl)
        {
            string signedInUserID = ClaimsPrincipal.Current.FindFirst(ClaimTypes.NameIdentifier).Value;
            var cache = new SessionADALCache(signedInUserID);

            AuthenticationManager authManager = new AuthenticationManager();
            ClientContext context = authManager.
        ...
        private static async Task<string> GetTokenForApplication(String serviceUri, TokenCache tokenCache)
        {
            ...
            AuthenticationResult authenticationResult = await authContext.AcquireTokenSilentAsync(serviceUri.ToString(), clientcred, new UserIdentifier(signedInUserID, UserIdentifierType.UniqueId));
